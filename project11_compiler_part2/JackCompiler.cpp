...
int main(int argc, char** argv)
{
    ...
    {
        std::cout << "Handling file: " << files[i] << std::endl;
        int slashPos = files[i].find('\\');
        std::string outfileName = files[i].substr(slashPos + 1, files[i].length() - slashPos - 5 - 1) + ".vm";
        JackTokenizer *tokenizer = new JackTokenizer(files[i]);
        SymbolTable *symboltable = new SymbolTable();
        CodeWriter *codewriter = new CodeWriter(outfileName);
        CompilationEngine *compilationEngine = new CompilationEngine(tokenizer, symboltable, codewriter);
        delete compilationEngine;
        std::filesystem::rename(outfileName, fileordir + '\\' + outfileName);
    }
